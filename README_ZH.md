...
- 检索 curated 文学知识图谱，目前覆盖 30 部常见教学作品、319 条 triples，包括作者、出版年份、文类、体裁、中心人物、主题和标题别名；
...
这里的主张不是“系统自动批改整篇论文”，而是展示一个更适合真实教学场景的 review-routing 机制：低风险局部语法修改可自动采纳，文学事实、论证和解释性改写保留给教师复核。

此外，已经用 DeepSeek、Qwen、GLM、Kimi 对前 10 个 benchmark 样例做了 live API validation。40 次 provider 调用均无 request error 和 parse error；生成的 43 条 live feedback decisions 中，35 条进入教师复核，41 条带有 KG-supported 标记。
